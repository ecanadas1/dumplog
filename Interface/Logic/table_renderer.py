...
class TableRenderer:
        ...
        with self.controller._ui_lock:
            ...
            def create_row_controls(line_num, row_data, is_context_row=False):
                ...
                for col_name in display_columns_order:
                    if col_name == "chk":
                        cells.append(ft.Container(content=ft.Checkbox(value=is_marked, on_change=self._handle_checkbox_change, data=line_num, scale=0.8), width=self.col_widths["chk"], alignment=ft.alignment.Alignment(-1, 0)))
                    else:
                        # Acceso dinámico a los datos de la fila y limpieza para display
                        col_val = self._get_display_value(getattr(row_data, col_name, ''))
                        
                        # Tratamiento especial para mensaje y nivel
                        if col_name == "mensaje":
                            raw_msg = col_val
                            ...
                            cells.append(ft.Container(content=ft.Text(col_val, size=11, color=level_color, weight=level_weight, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS, max_lines=1, text_align=ft.TextAlign.LEFT), width=self.col_widths.get(col_name, 85), alignment=ft.alignment.Alignment(-1, 0)))
                        else:
                            # Columnas estándar y extras
                            width = self.col_widths.get(col_name, 85) # Ancho por defecto para extras
                            cells.append(ft.Container(content=ft.Text(col_val, size=11, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS, max_lines=1, text_align=ft.TextAlign.LEFT, tooltip=col_val if col_val else None), width=width, alignment=ft.alignment.Alignment(-1, 0)))
    ...
    def _create_context_row_controls(self, line_num, row_data, display_columns_order, marked_lines, selected_lines):
        ...
        for col_name in display_columns_order:
            if col_name == "chk":
                cells.append(ft.Container(content=ft.Checkbox(value=is_marked, on_change=lambda e, ln=line_num: self.controller.toggle_mark(e, ln), data=line_num, scale=0.8), width=self.col_widths["chk"], alignment=ft.alignment.Alignment(-1, 0)))
            else:
                col_val = self._get_display_value(getattr(row_data, col_name, ''))
                if col_name == "mensaje":
                    raw_msg = col_val
                    ...
                    level_color = self.layout._get_level_color(col_val) if self.app_state.syntax_highlighting else None
                    cells.append(ft.Container(content=ft.Text(col_val, size=11, color=level_color, weight=ft.FontWeight.BOLD if line_num == target_line else None), width=self.col_widths.get(col_name, 85), alignment=ft.alignment.Alignment(-1, 0)))
                else:
                    width = self.col_widths.get(col_name, 85)
                    cells.append(ft.Container(content=ft.Text(col_val, size=11, weight=ft.FontWeight.BOLD if line_num == target_line else None, no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS, max_lines=1, text_align=ft.TextAlign.LEFT, tooltip=col_val if col_val else None), width=width, alignment=ft.alignment.Alignment(-1, 0)))
