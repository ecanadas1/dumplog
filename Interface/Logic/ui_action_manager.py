...
class UIActionManager:
    ...
    def on_dynamic_filter_search_change(self, e):
        ...
        # page.run_task() es thread-safe; asyncio.create_task() requería un loop activo
        # en el hilo llamante, lo que no está garantizado desde page.run_thread().
        self.mgr._filter_controls_debounce_task = self.mgr.page.run_task(self.mgr._debounce_update_filter_controls)
    ...
    def trigger_debounced_filter(self, reason: str):
        ...
        # page.run_task() es thread-safe; asyncio.create_task() requería un loop activo
        # en el hilo llamante, lo que no está garantizado desde page.run_thread().
        self.mgr._debounce_task = self.mgr.page.run_task(self._debounce_refresh)
