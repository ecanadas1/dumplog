# Interface/Core/config_manager.py
"""
Responsabilidad única: leer y escribir la configuración persistente en el archivo .ini.
No conoce ni la UI ni el estado de la aplicación.
"""
import configparser
from pathlib import Path
from .app_settings import AppSettings, SeccionAPP
import logging
...
class ConfigManager:
    ...
    def cargar(self) -> AppSettings:
        ...
        try:
            ...
            app_data = SeccionAPP(
                ...
                historial_busqueda=self._parser.get('APP', 'historial_busqueda', fallback='').split('|||')
                    if self._parser.get('APP', 'historial_busqueda', fallback='') else []
            )
    ...
    def _validar_page_size(page_size: int) -> int:
        if not (20 <= page_size <= 1000):
            logger.warning(f"Tamaño de página configurado '{page_size}' fuera de rango (20-1000). Usando '{DEFAULT_PAGE_SIZE}'.")
