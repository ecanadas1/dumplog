...
class AppLayout:
    ...
    def _init_controls(self):
        ...
        # Etiquetas con nombre para el cambio de idioma (evita acceso por índice frágil)
        self.start_label = ft.Text(f"{i18n.t('sidebar.start')}:", weight="bold", size=12, width=50)
        self.end_label   = ft.Text(f"{i18n.t('sidebar.end')}:",   weight="bold", size=12, width=50)

        self.start_controls = ft.Column([
            ft.Row([self.start_label, self.btn_date_start, self.btn_time_start]),
            self.start_date_label
        ], spacing=0)

        self.end_controls = ft.Column([
            ft.Row([self.end_label, self.btn_date_end, self.btn_time_end]),
            self.end_date_label
        ], spacing=0)
        ...
        self.filter_controls: dict = {}
        # Referencias directas para el cambio de idioma (evita índices frágiles)
        self.filter_search_fields: dict = {}       # {col_name: ft.TextField}
        self.filter_panel_title_texts: dict = {}   # {col_name: ft.Text}
    ...
    def refresh_controls(self):
        ...
        self.start_label.value = f"{i18n.t('sidebar.start')}:"
        self.end_label.value   = f"{i18n.t('sidebar.end')}:"
    ...
    def crear_filtros_iniciales(self, unique_data: dict):
        ...
        self.filter_search_fields = {}
        self.filter_panel_title_texts = {}
        ...
        for col_name, values in unique_data.items():
            ...
            # Campo de búsqueda interno — guardamos referencia directa
            ...
            self.filter_search_fields[col_name] = search_field
            
            title = translation_map.get(col_name, col_name.capitalize())
            title_text = ft.Text(title, weight=ft.FontWeight.BOLD)
            self.filter_panel_title_texts[col_name] = title_text  # referencia directa al texto
