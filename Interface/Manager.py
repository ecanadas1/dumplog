...
class Manager:
    ...
    def _refresh_ui_texts_only(self):
        ...
        l.start_controls.controls[0].controls[0].value = f"{i18n.t('sidebar.start')}:"
        l.end_controls.controls[0].controls[0].value = f"{i18n.t('sidebar.end')}:"
        ...
        if l.dynamic_filters_column.controls:
            for panel in l.dynamic_filters_column.controls:
                if isinstance(panel, ft.ExpansionTile):
                    search_field = panel.controls[0].controls[0]
                    col_name = search_field.data
                    panel.title.value = translation_map.get(col_name, col_name.capitalize())
    ...
    async def copy_selected_lines(self, e=None): await self.selection_mgr.copy_selected_lines(e)
    ...
    async def select_by_search(self, query: str): await self.selection_mgr.select_by_search(query)
        ...
        try:
            ...
            try:
                if self.app_state.use_regex:
                    self.layout.search_field.error_text = "Regex inválida" if self.app_state.regex_error else None
    ...
    async def goto_line(self, e): await self.selection_mgr.goto_line(e)
    async def goto_next_marked(self, e): await self.selection_mgr.navigate_marked(1)
    async def goto_prev_marked(self, e): await self.selection_mgr.navigate_marked(-1)
    async def _jump_to_line(self, line_num: int): await self.selection_mgr.jump_to_line(line_num)
