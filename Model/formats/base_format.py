...
class SimpleFormat(BaseFormat):
    def __init__(self, config: Dict[str, Any], notes_rules_loader=None):
        ...
        try:
            self._regex = re.compile(self._regex_str) if self._regex_str else re.compile(r"$.")
        except re.error as e:
            logger.error(f"Error al compilar regex para el formato '{self._id}': {e}")
            self._regex = re.compile(r"$.")  # Expresión regular que nunca coincide
    ...
    def parse_line(self, line: str, line_number: int) -> Optional[LogEntry]:
        ...
        try:
            ...
            if isinstance(ts_idx, list):
                ts_str = " ".join(match.group(idx) for idx in ts_idx if match.group(idx) is not None)
            ...
            if ts_str:
                if "%Y" not in self._date_format and "%y" not in self._date_format:
                    ...
                    ts_str_with_year = f"{current_year} {ts_str_clean}"
                    date_fmt = f"%Y {self._date_format}"
                    timestamp = datetime.strptime(ts_str_with_year, date_fmt)
                    # Si la fecha resultante es futura, probablemente corresponde al año anterior
                    if timestamp > datetime.now():
                         try:
                             timestamp = timestamp.replace(year=timestamp.year - 1)
                         except ValueError:
                             # Manejo seguro para año bisiesto (29 de febrero)
            ...
            if nivel_idx is not None:
                raw_nivel = match.group(nivel_idx)
                if raw_nivel is not None:
                    nivel = raw_nivel.strip()
                    # Normalizar niveles de log para la UI
                    nivel_upper = nivel.upper()
                    if nivel_upper in ["I", "INF", "INFO"]:
                        nivel = "INFO"
                    elif nivel_upper in ["W", "WRN", "WARN", "WARNING"]:
                        nivel = "WARNING"
                    elif nivel_upper in ["E", "ERR", "ERROR", "CRIT", "CRITICAL", "FATAL"]:
                        nivel = "ERROR"
                    elif nivel_upper in ["D", "DEB", "DEBUG"]:
                        nivel = "DEBUG"
                else:
                    nivel = "INFO"
            else:
            ...
            for col in self._extra_columns:
                ...
                if col_idx is not None:
                    val = match.group(col_idx)
                    extras[col] = val if val is not None else ""
            ...
            logger.debug(f"Error parseando la línea {line_number} con formato '{self._id}': {e}", exc_info=True)
            return None
