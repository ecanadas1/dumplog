...
class LogParser:
    ...
    def load_rules(cls, config_path="notes_config.json"):
        ...
        from .notes_registry import load_rules
        cls.notes_rules = load_rules(config_path)
            ...
            with open(file_path, 'r', encoding='utf-8', errors='replace') as file:
                for line_number, line in enumerate(file, start=1):
                    ...
                    try:
                        ...
                        logger.error(f"Error parseando la línea {line_number} con formato '{self.formato.name}': {e}")
                        entry = None
    ...
    def parse_file_to_dataframe(self, file_path: str) -> pd.DataFrame:
        """
        Lee un archivo de log y construye el DataFrame en un único paso,
        sin crear la lista intermedia de LogEntry.

        Elimina el doble pico de memoria (lista LogEntry + DataFrame) que
        se producía al llamar a parse_file() seguido de to_dataframe().
        El ahorro es de aproximadamente un 50 % de RAM en ficheros grandes.

        Si el formato activo sobreescribe post_load_hook con lógica propia,
        se cae automáticamente al método legacy para mantener compatibilidad.

        Args:
            file_path (str): Ruta al archivo de log.

        Returns:
            pd.DataFrame: DataFrame con las entradas parseadas, o vacío si hay error.
        """
        from .formats.base_format import BaseFormat

        # Compatibilidad: si el formato tiene un hook personalizado, usar método legacy
        if type(self.formato).post_load_hook is not BaseFormat.post_load_hook:
            logger.info(
                f"Formato '{self.formato.name}' usa post_load_hook personalizado. "
                "Usando método legacy (parse_file + to_dataframe)."
            )
            entries = self.parse_file(file_path)
            return LogParser.to_dataframe(entries)

        logger.info(f"Iniciando parseo directo a DataFrame ({self.formato.name}): {file_path}")

        # Listas de columnas base: se rellenan línea a línea sin crear LogEntry
        col_linea: list = []
        col_timestamp: list = []
        col_nivel: list = []
        col_mensaje: list = []
        ...
        # Columnas extra dinámicas: {nombre_col → lista_de_valores}
        # Se expanden al descubrir nuevas claves; las filas anteriores se rellenan con ''
        extras_cols: dict = {}
        invalid_count = 0
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='replace') as file:
                for line_number, line in enumerate(file, start=1):
                    ...
                    # Índice de la fila actual (antes de añadir la nueva)
                    row_idx = len(col_linea)

                    col_linea.append(entry.linea)
                    col_timestamp.append(entry.timestamp)
                    col_nivel.append(entry.nivel)
                    col_mensaje.append(entry.mensaje)
                    col_notas.append(entry.notas)

                    # Añadir '' a todas las columnas extra ya conocidas para esta fila
                    for key in extras_cols:
                        extras_cols[key].append('')

                    # Actualizar con los valores reales de la entrada
                    if entry.extras:
                        for key, val in entry.extras.items():
                            str_val = val if val is not None else ''
                            if key not in extras_cols:
                                # Nueva columna: rellenar filas anteriores con ''
                                extras_cols[key] = [''] * row_idx
                                extras_cols[key].append(str_val)
                            else:
                                # Sobreescribir el '' añadido en el bloque anterior
                                extras_cols[key][-1] = str_val

            total_entries = len(col_linea)
            logger.info(
                f"Parseo completado: {total_entries} entradas válidas, "
                f"{invalid_count} líneas inválidas"
            )

            if total_entries == 0:
                return pd.DataFrame()

            # Construir el DataFrame de una sola vez con todas las columnas
            data = {
                'linea': col_linea,
            ...
            # Liberar las listas de columnas inmediatamente para recuperar memoria
            del col_linea, col_timestamp, col_nivel, col_mensaje, col_notas
            del extras_cols, data

            # Optimizaciones de tipo (igual que to_dataframe)
    ...
    def to_dataframe(entries: List[LogEntry]) -> pd.DataFrame:
        ...
        Método legacy mantenido para compatibilidad con formatos que usen
        post_load_hook personalizado. Para el flujo normal de carga, usar
        parse_file_to_dataframe() que es más eficiente en memoria.
        """
